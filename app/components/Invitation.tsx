    ...
    <div style={{ userSelect: 'none' }}>
          ...
          return (
            <div key={i} onClick={() => step !== 0 && onSelect(i)}
              style={{
                ...
                boxShadow: step === 0 ? '0 16px 48px rgba(0,150,255,0.35)' : '0 8px 24px rgba(0,0,0,0.2)',
              ...
              {absStep > 0 && (
                <div style={{ position: 'absolute', inset: 0, background: `rgba(5,6,10,${absStep * 0.18})`, borderRadius: 16 }} />
              )}
            </div>
          );
        })}
      ...
      <div className="flex items-center justify-center gap-6 mt-2">
        <button onClick={prev} aria-label="Previous"
          style={{ background: 'none', border: '1px solid #7099c0', borderRadius: '50%', width: 34, height: 34, cursor: 'pointer', color: '#0096FF', fontSize: 16, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>‹</button>
        <div className="flex gap-2">
          {images.map((_, i) => (
            <div key={i} onClick={() => onSelect(i)}
              style={{ width: i === activeIdx ? 18 : 8, height: 8, borderRadius: 999, background: i === activeIdx ? '#0096FF' : '#a0c0e8', cursor: 'pointer', transition: 'all 0.3s ease' }} />
          ))}
        </div>
...
const SCHEDULE = [
  { time: '15:00', label: 'Welcome Guests' },
  { time: '16:00', label: 'Reception Begins' },
  { time: '16:30', label: 'Engagement Ceremony' },
  { time: '17:00', label: 'Cake Cutting & Toast' },
  { time: '20:30', label: 'Farewell' },
];
const HIGHLIGHT_DAY = 4;

const GUESTBOOK_SAMPLE = [
  { name: 'Henry Coleman',   date: '7/8/2026 21:00 PM', msg: 'Congratulations on your engagement! So thrilled to share this moment with you.' },
  { name: 'Grace Mitchell',  date: '7/8/2026 21:06 PM', msg: 'What a beautiful couple. May your journey together be forever bright!' },
  { name: 'William Foster',  date: '7/8/2026 21:09 PM', msg: 'Wishing Von Nak and Barn Ravan all the happiness in the world. Love you both!' },
  { name: 'Charlotte Reyes', date: '7/8/2026 21:12 PM', msg: 'Here is to love, laughter, and happily ever after. Congratulations!' },
  { name: 'James Hollis',    date: '7/8/2026 21:15 PM', msg: 'Congrats you two! Cannot wait to celebrate this special day with you.' },
];
...
useEffect(() => {
  ...
  const submitWish = () => {
    ...
    const now = new Date();
    const d = `${now.getMonth()+1}/${now.getDate()}/${now.getFullYear()} ${now.getHours()}:${String(now.getMinutes()).padStart(2,'0')} ${now.getHours()>=12?'PM':'AM'}`;
    setWishes([{ name: wishName, date: d, msg: wishMsg }, ...wishes]);
    setWishName(''); setWishMsg('');
  };

  const sectionLabel: React.CSSProperties = {
    fontSize: 15, letterSpacing: '0.3em', textTransform: 'uppercase',
    color: '#0066CC', ...F, textAlign: 'center', marginBottom: 16,
  };
  const darkCard: React.CSSProperties = {
    background: '#0096FF', borderRadius: 14,
    padding: '32px 28px', position: 'relative', overflow: 'hidden',
  };

  return (
    <div className="relative" style={{ background: '#f5ede6', minHeight: '100vh', ...F, overflowX: 'hidden' }}>

      {/* Blue background (landing only) */}
      {!isOpened && (
        <>
          <div className="fixed inset-0" style={{ background: 'linear-gradient(135deg,#0066CC 0%,#0096FF 50%,#0066CC 100%)', zIndex: 0 }} />
          {HEARTS.map((h, i) => (
        ...
        <main className="flex items-center justify-center min-h-screen px-4 relative z-10">
          <div className={`flex flex-col items-center gap-4 transition-all duration-700 ${isLoaded ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}
            ...
            <div className="relative w-full"
              style={{ background: '#f5ede6', borderRadius: 18, boxShadow: '0 20px 60px rgba(0,0,0,0.45)', padding: '40px 48px 36px', overflow: 'visible' }}>
              ...
              <div className="flex justify-center mb-5">
                <div className="flex items-center justify-center rounded-full"
                  style={{ width: 52, height: 52, background: '#0066CC', boxShadow: '0 4px 14px rgba(0,150,255,0.5)' }}>
              ...
              <div className="text-center" style={{ lineHeight: 1.15 }}>
                <p className="text-4xl md:text-5xl" style={{ color: '#0096FF', fontWeight: 300 }}>Von Nak</p>
                <p className="text-2xl my-1" style={{ color: '#0096FF', fontWeight: 300 }}>&amp;</p>
                <p className="text-4xl md:text-5xl" style={{ color: '#0096FF', fontWeight: 300 }}>Barn Ravan</p>
              </div>

              {/* Divider */}
              <div className="flex items-center justify-center gap-3 my-5">
                <div style={{ flex: 1, height: 1, background: '#7099c0', opacity: 0.5 }} />
                <span style={{ color: '#406680', fontSize: 16 }}>❧</span>
                <div style={{ flex: 1, height: 1, background: '#7099c0', opacity: 0.5 }} />
              </div>

              <p className="text-center text-base" style={{ color: '#0096FF', letterSpacing: '0.1em' }}>04, October 2026</p>
              <p className="text-center text-sm mt-2" style={{ color: '#305060', letterSpacing: '0.08em' }}>Cordially Invites</p>

              {guestName && (
                <p className="text-center mt-1" style={{ color: '#0096FF', fontSize: 25, fontWeight: 600, fontFamily: 'var(--font-khmer), var(--font-poppins), sans-serif' }}>
                  {guestName}
                </p>
              )}
              ...
              <div className="flex justify-center mt-6">
                <button onClick={handleOpen}
                  style={{ background: '#0066CC', color: '#fff', border: 'none', borderRadius: 999, padding: '11px 44px', fontSize: 15, letterSpacing: '0.1em', cursor: 'pointer', boxShadow: '0 4px 18px rgba(0,150,255,0.45)', transform: buttonPressed ? 'scale(0.96)' : 'scale(1)', transition: 'transform 0.15s' }}>
                  Open
                </button>
              </div>
            </div>{/* end cream card */}
        ...
        <div className="animate-in fade-in duration-700" style={{ background: '#f5ede6', overflowX: 'hidden' }}>
          <div className="mx-auto" style={{ maxWidth: 480, overflowX: 'hidden' }}>
            ...
            <section className="pb-4 px-4" style={{ paddingTop: '22%', overflowX: 'clip', overflowY: 'visible' }}>
              <p style={sectionLabel}>Save the Date</p>
              <div className="relative w-full" style={{ overflow: 'visible' }}>
                <Image src="/images/envelope-background.png" alt="Envelope" width={1013} height={1168} className="w-full h-auto" priority />
                ...
                <div className="absolute pointer-events-none" style={{ bottom: 0, left: 0, right: 0, height: '57.2%', zIndex: 20 }}>
                  <Image src="/images/envelope-cover.png" alt="" fill className="object-fill" />
                </div>
            ...
            <section className="py-8 px-4 text-center">
              <p className="text-4xl" style={{ color: '#0096FF', fontWeight: 300, fontStyle: 'italic' }}>Von Nak</p>
              <p className="text-xl my-1" style={{ color: '#6099CC' }}>❧</p>
              <p className="text-4xl" style={{ color: '#0096FF', fontWeight: 300, fontStyle: 'italic' }}>Barn Ravan</p>
            </section>
            ...
            <section className="px-4 pb-6">
              <div style={darkCard} className="relative">
                ...
                <p style={{ ...sectionLabel, color: '#e8c8c8', marginBottom: 24 }}>Engagement Ceremony</p>
                ...
                <div className="flex items-stretch" style={{ position: 'relative', zIndex: 3, marginBottom: 20 }}>
                  ...
                  <div className="flex flex-col items-center justify-center"
                    style={{ minWidth: 88, borderRight: '1px solid rgba(255,255,255,0.15)', paddingRight: 20 }}>
                    <p style={{ fontSize: 68, fontWeight: 700, color: '#fff', lineHeight: 1, letterSpacing: '-2px' }}>04</p>
                    <p style={{ fontSize: 11, color: '#e8b4b4', letterSpacing: '0.2em', marginTop: 4 }}>OCT</p>
                    <p style={{ fontSize: 11, color: '#e8c8c8' }}>2026</p>
                  </div>

                  {/* Right — details */}
                  <div className="flex flex-col justify-center gap-3" style={{ paddingLeft: 20, flex: 1 }}>
                    <div>
                      <p style={{ fontSize: 9, letterSpacing: '0.25em', color: '#e8b4b4', textTransform: 'uppercase', marginBottom: 2 }}>Day</p>
                      <p style={{ fontSize: 14, color: '#fff', fontWeight: 500 }}>Sunday</p>
                    </div>
                    <div style={{ height: 1, background: 'rgba(255,255,255,0.1)' }} />
                    <div>
                      <p style={{ fontSize: 9, letterSpacing: '0.25em', color: '#e8b4b4', textTransform: 'uppercase', marginBottom: 2 }}>Time</p>
                      <p style={{ fontSize: 14, color: '#fff', fontWeight: 500 }}>08:00 PM</p>
                    </div>
                    <div style={{ height: 1, background: 'rgba(255,255,255,0.1)' }} />
                    <div>
                      <p style={{ fontSize: 9, letterSpacing: '0.25em', color: '#e8b4b4', textTransform: 'uppercase', marginBottom: 2 }}>Location</p>
                      <p style={{ fontSize: 13, color: '#fff', fontWeight: 500 }}>Prey Sdau</p>
                      <p style={{ fontSize: 11, color: '#e8c8c8' }}>Phnom Penh, Cambodia</p>
                    </div>
                  </div>
                </div>
            ...
            <section className="px-4 pb-8">
              ...
              <div style={{ background: '#fff', borderRadius: 14, overflow: 'hidden', boxShadow: '0 4px 24px rgba(90,10,18,0.1)' }}>

                {/* Map preview — static embed placeholder with pin overlay */}
                <div style={{ position: 'relative', width: '100%', height: 160, background: '#e8e0d8' }}>
                  <iframe
                    title="Venue Map"
                  ...
                  <div style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -100%)', pointerEvents: 'none', zIndex: 5 }}>
                    <div style={{ width: 28, height: 28, background: '#0096FF', borderRadius: '50% 50% 50% 0', transform: 'rotate(-45deg)', boxShadow: '0 2px 8px rgba(0,150,255,0.45)' }} />
                  </div>
                </div>
                ...
                <div style={{ padding: '16px 20px 20px' }}>
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-shrink-0 mt-1">
                      <svg width="18" height="18" viewBox="0 0 24 24" fill="none">
                        <path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7z" fill="#0096FF" />
                        <circle cx="12" cy="9" r="2.5" fill="#fff" />
                      </svg>
                    </div>
                    <div>
                      <p className="font-khmer" style={{ fontWeight: 600, fontSize: 14, color: '#0d1a2d', marginBottom: 2, fontFamily: 'var(--font-khmer), var(--font-poppins), sans-serif' }}>
                        វត្តសិរីសុខារកាដុះ
                      </p>
                      <p style={{ fontSize: 12, color: '#406080', lineHeight: 1.5 }}>
                  ...
                  <a
                    ...
                    style={{
                      ...
                      background: '#0096FF', color: '#fff', borderRadius: 999,
                      fontSize: 12, letterSpacing: '0.1em', textDecoration: 'none',
                      boxShadow: '0 3px 12px rgba(0,150,255,0.3)',
                    }}
                  >
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none">
                      <path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7z" fill="white" />
                      <circle cx="12" cy="9" r="2.5" fill="#0096FF" />
                    </svg>
                    Get Directions
                  </a>
                </div>
            ...
            {/* <section className="px-4 pb-6">
              <div style={darkCard} className="relative">
                ...
                <p style={{ ...sectionLabel, color: '#e8c8c8', marginBottom: 24 }}>Wedding Day Schedule</p>
                <div className="space-y-4" style={{ position: 'relative', zIndex: 3 }}>
                  {SCHEDULE.map((s, i) => (
                    <div key={i} className="flex items-center gap-4">
                      <span style={{ color: '#e8c8c8', fontSize: 13, minWidth: 42, fontVariantNumeric: 'tabular-nums' }}>{s.time}</span>
                      <div className="rounded-full flex-shrink-0" style={{ width: 8, height: 8, background: '#e8b4b4' }} />
        ...
        {isPlaying && (
          <>
            ...
            <div style={{
              ...
              animation: 'musicPulse 1.8s ease-out infinite 0.5s',
            }} />
          </>
        )}
        <button
          ...
          style={{
            ...
            background: isPlaying
              ? 'linear-gradient(135deg, #0096FF, #00AAFF)'
              : 'rgba(0,102,204,0.82)',
            border: '2px solid rgba(255,255,255,0.22)',
            backdropFilter: 'blur(10px)',
            boxShadow: '0 4px 20px rgba(0,150,255,0.55)',
            cursor: 'pointer',
